...
let
  versionData = builtins.fromJSON (builtins.readFile ../amp/hashes.json);
...
stdenv.mkDerivation {
  ...
  meta = with lib; {
    description = "CLI for Amp, an agentic coding tool in research preview from Sourcegraph (binary version, for plugin support)";
