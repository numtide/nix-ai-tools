...
latest_version=$(npm view @motesoftware/nanocoder version)
...
# Update URL
sed -i "s|/@motesoftware/nanocoder/-/nanocoder-[^/]\\+\\.tgz|/@motesoftware/nanocoder/-/nanocoder-${latest_version}.tgz|" "$package_file"

# Get new source hash
echo "Getting new source hash..."
new_hash=$(nix-prefetch-url "https://registry.npmjs.org/@motesoftware/nanocoder/-/nanocoder-${latest_version}.tgz" | tail -1)
...
awk -v new_hash="$new_sri_hash" '
  /url = "https:\/\/registry\.npmjs\.org\/@motesoftware\/nanocoder/ { found=1 }
  found && /hash = / {
...
# Extract the tarball
curl -sL "https://registry.npmjs.org/@motesoftware/nanocoder/-/nanocoder-${latest_version}.tgz" | tar -xz -C "$temp_dir"
...
if ! npm_deps_output=$(nix build "$script_dir/../.."#nanocoder 2>&1); then
...
nix build "$script_dir/../.."#nanocoder

echo "Update completed successfully!"
echo "nanocoder has been updated from $current_version to $latest_version"
